...
export class ReleaseService {
  ...
  async runReleaseWatcher() {
    ...
    this.logger.log(`Starting Release Watcher. Current time: ${time}`);
    try {
      ...
      const artistIds = await artists.map(a => a.id); //Spotify
      const dBArtistIds = await this.artist.getArtistIds(); //Database
      const albumIds = await this.artist.getAlbumIds(); //Database
  ...
  //   const albumIds = await this.artist.getAlbumIds();
  //   const newReleases = await this.spotify.getNewReleases(
  //     artistIds,
  //     artistsNames,
  //     albumNames,
